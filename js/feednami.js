...
(function() {
    var FeednamiClientRequest = function() {
        function FeednamiClientRequest(client, url, options, callback) {
            _classCallCheck(this, FeednamiClientRequest);
            this.client = client;
            this.url = url;
        ...
        FeednamiClientRequest.prototype.send = function send(callback) {
            var endpoint = window.feednamiEndpoint || 'https://api.feednami.com';
            var apiRoot = endpoint + '/api/v1.1';
            var feedUrl = this.url;
            var options = this.options;
            var qs = 'url=' + encodeURIComponent(feedUrl);
            if (this.client.publicApiKey) {
            ...
            if ('fetch'in window) {
                ...
                (function() {
                    ...
                    var callbackName = 'jsonp_callback_' + new Date().getTime() + '_' + Math.round(1000000 * Math.random());
    ...
    var FeednamiClient = function() {
        function FeednamiClient() {
            _classCallCheck(this, FeednamiClient);
            this.promisePolyfillCallbacks = [];
            this.promiseLoaded = 'Promise'in window
        }
        FeednamiClient.prototype.loadPolyfills = function loadPolyfills(callback) {
            if (this.promiseLoaded) {
                callback()
            } else {
                this.promisePolyfillCallbacks.push(callback);
                if (!this.polyfillScriptsAdded) {
                    this.polyfillScriptsAdded = true;
                    var script = document.createElement('script');
                    script.src = 'https://feednami-static.storage.googleapis.com/js/v1.1/promise.js';
        ...
        FeednamiClient.prototype.loadPromiseCallback = function loadPromiseCallback() {
            this.promiseLoaded = true;
            for (var _iterator = this.promisePolyfillCallbacks, _isArray = Array.isArray(_iterator), _i = 0, _iterator = _isArray ? _iterator : _iterator[Symbol.iterator](); ; ) {
                var _ref;
                if (_isArray) {
        ...
        FeednamiClient.prototype.load = function load(url, options, callback) {
            ...
            var request = new FeednamiClientRequest(this,url,options,callback);
            return request.run()
        }
        ;
